...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["med_name"])) {
    ...
    if ($med_name !== "" && $dosage !== "" && $schedule_date !== "") {
        try {
            ...
            $stmt = $pdo->prepare("INSERT INTO agenda_medicamentos (usuario_id, medicamento_id, dosagem, data_agendada) VALUES (?, ?, ?, ?)");
...
if (isset($_POST["delete_agenda_id"])) {
    ...
    try {
        $stmt = $pdo->prepare("DELETE FROM agenda_medicamentos WHERE id = ? AND usuario_id = ?");
...
$agendamentos = [];
try {
    $stmt = $pdo->prepare("SELECT ag.id as agenda_id, ag.dosagem, ag.data_agendada, m.nome as med_nome, m.descricao, m.imagem, m.link_compra FROM agenda_medicamentos ag JOIN medicamentos m ON ag.medicamento_id = m.id WHERE ag.usuario_id = ? ORDER BY ag.data_agendada ASC");
...
// Busca todos os medicamentos disponíveis (para a seção 'Medicamentos Populares')
$medicamentos_disponiveis = [];
try {
    $stmt = $pdo->query("SELECT id, nome, descricao, imagem, preco, link_compra FROM medicamentos ORDER BY nome ASC");
    $medicamentos_disponiveis = $stmt->fetchAll(PDO::FETCH_ASSOC);
} catch (PDOException $e) {
    echo "<div class=\"error-message\">Erro ao carregar medicamentos: " . $e->getMessage() . "</div>";
}
    ...
    <div class="container">
        ...
        <section id="medicamentos" class="fade-in">
            ...
            <p class="text-center" style="color: var(--text-secondary); margin-bottom: 2rem;">
                Explore e adicione medicamentos diretamente ao seu carrinho
            </p>
            
            <div class="meds-grid">
                <?php foreach ($medicamentos_disponiveis as $med): ?>
                    <div class="med-card fade-in">
                        <h3 class="med-name"><?= htmlspecialchars($med["nome"]) ?></h3>
                        
                        <?php if (!empty($med["imagem"])): ?>
                            <img src="<?= htmlspecialchars($med["imagem"]) ?>" alt="Imagem de <?= htmlspecialchars($med["nome"]) ?>" class="med-image">
                        <?php endif; ?>

                        <p class="med-price">R$ <?= number_format($med["preco"], 2, ",", ".") ?></p>
                        ...
                        <div class="med-actions">
                            <form action="carrinho.php" method="post" style="display: inline;">
                                <input type="hidden" name="add_to_cart" value="<?= $med["id"] ?>">
